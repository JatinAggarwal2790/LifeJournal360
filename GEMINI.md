...
## Version Logging Requirement (CRITICAL)
Whenever any modifications are made, features added, bugs fixed, or a new version is created in this repository:
1. You **MUST** append an entry to `/version-log.txt` documenting the changes.
2. Format specifications:
   - Each version entry must be **<= 150 words**.
   - Use a clear version header (e.g. `Version X.Y.Z — Short Descriptive Title`).
   - Describe all changes using **bulleted headers** (e.g., `* **Header**: Description`).
   - Keep entries strictly factual, concise, and focused on functional/architectural deliverables.
